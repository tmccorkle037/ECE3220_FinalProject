...
public:
    ...
    virtual void feedback() = 0;    //pure virtual function
};
...
public:
    ...
    int error; //error for if its a new profile
};
void Student::newStudent(string cardInfo)
{
    ...
    status = "Student";
    purchaseSwipes();
    output.open(idNum + ".txt");    //creates new file for student
    output << status << "\n" << swipes << "\n" << firstName << " " << lastName;
    output.close();
    error = 0;
}
...
private:
    ...
    double hours;
public:
    string password;    //public so that main can access it
...
void Employee::updateFile(){    //update file after hours have been added
    ofstream outFile;
...
void help(){
    system("clear");
    cout << "--HELP MENU--" << endl;
    cout << "If you're a student:\nLet us know you're a student, then swipe your card so we can access your account information. Next you will be able to buy food, buy swipes, or leave feedback about your dining experience. Follow the on screen commands to make your menu options." << endl;
...
int main() {
    ...
    while(userLevel >3 || userLevel < 0){   //error checks to make sure number is valid
    ...
    if(userLevel == 1){ //student user
        ...
        system("clear");    //clear screen so you can't see card info
        try{
            Student *s1 = new Student(parsedCardInfo);
            while(choice !=4)
            {
                studentMenu();
                if(s1->error == 1)  //the user is a new student, must create a new file
                {
                    choice = 5;
                }
                else{
        ...
        try{    //try making a manager object
            ...
            system("clear");    //clears screen so you can't see the password
            ...
            delete manager; //deallocate memory
        }
        catch(int e){
